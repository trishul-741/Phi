...
class ContentBERT(nn.Module):
    ...
    def __init__(self):
        ...
        # BERT parameters are left unfrozen here; discriminative fine-tuning
        # (layer-wise LR decay) will control adaptation via the optimizer.
    ...
    def forward(self, input_ids, attention_mask):
        ...
        # Removed explicit unfreeze_layer method.
        # Now relying on layer-wise learning rates (discriminative fine-tuning)
...
class StatFF(nn.Module):
    ...
    def __init__(self, input_dim: int):
        ...
        self.net = nn.Sequential(
            ...
            nn.BatchNorm1d(cfg.STAT_OUT_DIM), # 2nd layer BatchNorm prevents scaling issues vs BERT vector
            nn.ReLU(inplace=True),
...
# ══════════════════════════════════════════════════════════════════════
# PhishGuardNet — Full Non-Visual Model
# ══════════════════════════════════════════════════════════════════════

class PhishGuardNet(nn.Module):
    """
    Multimodal phishing classifier combining ContentBERT + LexicalCharCNN + StatFF
    via Multi-Head Cross-Attention.

    Non-visual fusion classifier: _init_weights() skips ContentBERT to preserve pretrained weights.
    ...
    def set_dropout(self, p: float):
        """
        Dynamically update dropout probability for warm dropout scheduling.
        Maintains the proportional scaling for layers that use p * 0.5.
        """
        for name, module in self.named_modules():
            if isinstance(module, nn.Dropout):
                # We identify half-dropouts by checking if they are the 2nd dropout 
                # in StatFF or Fusion blocks (which use DROPOUT * 0.5 in init).
                # A robust heuristic: if the current p is exactly half of the config DROPOUT, scale identically.
                is_half = (module.p > 0 and abs(module.p - (cfg.DROPOUT * 0.5)) < 1e-5)
                module.p = p * 0.5 if is_half else p
