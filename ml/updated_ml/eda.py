...
# v5 assertions: binary/count features replaced with density/ratio
assert "form_action_density" in ALL_FEATURE_NAMES, (
    "ALL_FEATURE_NAMES does not contain 'form_action_density'. "
    "Check that features.py v5 changes have been applied (has_form_action was renamed)."
)
assert "has_form_action" not in ALL_FEATURE_NAMES, (
    "'has_form_action' still in ALL_FEATURE_NAMES — should be 'form_action_density'. "
    "Update features.py."
)
assert "external_link_ratio" in ALL_FEATURE_NAMES, (
    "ALL_FEATURE_NAMES does not contain 'external_link_ratio'. "
    "Check that features.py v5 changes have been applied (num_external_links was renamed)."
)
assert "num_external_links" not in ALL_FEATURE_NAMES, (
    "'num_external_links' still in ALL_FEATURE_NAMES — should be 'external_link_ratio'. "
    "Update features.py."
)
assert "subdomain_depth" in ALL_FEATURE_NAMES, (
    "ALL_FEATURE_NAMES does not contain 'subdomain_depth'. "
    "Check that features.py v5 changes have been applied."
)
assert "brand_impersonation_score" in ALL_FEATURE_NAMES, (
    "ALL_FEATURE_NAMES does not contain 'brand_impersonation_score'. "
    "Check that features.py v5 changes have been applied."
)

assert "has_password_field" in ALL_FEATURE_NAMES, (
    "ALL_FEATURE_NAMES missing has_password_field."
)
assert "title_mismatch" in ALL_FEATURE_NAMES, (
    "ALL_FEATURE_NAMES missing title_mismatch."
)
...
def plot_false_positive_analysis(val_df: pd.DataFrame, predictions: pd.DataFrame):
    """Generate 4 plots for false positive error analysis (Source, URL length, Feature impact, Confidence)."""
    if len(val_df) != len(predictions):
        logger.warning(f"Length mismatch: val_df ({len(val_df)}) vs predictions ({len(predictions)}). Skpping FP analysis.")
        return

    eval_df = val_df.copy().reset_index(drop=True)
    preds   = predictions.reset_index(drop=True)
    
    prob_col = "confidence" if "confidence" in preds.columns else ("prob" if "prob" in preds.columns else ("score" if "score" in preds.columns else None))
    pred_col = "pred_label" if "pred_label" in preds.columns else ("prediction" if "prediction" in preds.columns else ("label" if "label" in preds.columns else None))
    
    if not prob_col or not pred_col:
        logger.warning(f"Missing required probability/prediction columns in CSV. Found: {preds.columns}")
        return
        
    eval_df["confidence"] = preds[prob_col]
    eval_df["pred_label"] = preds[pred_col]
    
    eval_df["is_fp"] = (eval_df["label"] == 0) & (eval_df["pred_label"] == 1)
    eval_df["is_tn"] = (eval_df["label"] == 0) & (eval_df["pred_label"] == 0)
    
    # ── 1. FALSE POSITIVE BREAKDOWN BY SOURCE ──
    source_stats = []
    for source, group in eval_df[eval_df["label"] == 0].groupby("source"):
        fp_count = group["is_fp"].sum()
        total    = len(group)
        source_stats.append({"source": source, "fp_rate": fp_count / total, "fp_count": fp_count})
        
    if source_stats:
        df_src = pd.DataFrame(source_stats).sort_values("fp_rate", ascending=False)
        fig, ax = plt.subplots(figsize=(10, max(5, len(df_src) * 0.4)))
        ax.barh(df_src["source"], df_src["fp_rate"], color=COLORS["phishing"])
        ax.set_title("False Positive Rate by Source (Legitimate URLs)", fontsize=16, fontweight="bold")
        ax.set_xlabel("FP Rate")
        _save(fig, "07_fp_by_source")
    
    fp_len = eval_df[eval_df["is_fp"]]["url"].str.len()
    tn_len = eval_df[eval_df["is_tn"]]["url"].str.len()
    
    # ── 2. FALSE POSITIVE URL LENGTH DISTRIBUTION ──
    if len(fp_len) > 0 and len(tn_len) > 0:
        fig, ax = plt.subplots(figsize=(10, 5))
        ax.hist(tn_len, bins=50, alpha=0.6, label="True Negatives", color=COLORS["legitimate"], density=True)
        ax.hist(fp_len, bins=50, alpha=0.6, label="False Positives", color=COLORS["phishing"], density=True)
        ax.set_title("URL Length Distribution (Legitimate Sites)", fontsize=16, fontweight="bold")
        ax.set_xlabel("URL Length")
        ax.legend()
        _save(fig, "08_fp_url_lengths")
    
    # ── 3. TOP FEATURES IN FALSE POSITIVES ──
    available_feats = [f for f in ALL_FEATURE_NAMES if f in eval_df.columns]
    if available_feats and len(fp_len) > 0 and len(tn_len) > 0:
        fp_means = eval_df[eval_df["is_fp"]][available_feats].mean()
        tn_means = eval_df[eval_df["is_tn"]][available_feats].mean()
        std_devs = eval_df[eval_df["label"] == 0][available_feats].std().replace(0, 1)
        
        delta = ((fp_means - tn_means) / std_devs).sort_values()
        
        fig, ax = plt.subplots(figsize=(10, max(6, len(delta) * 0.4)))
        colors = [COLORS["legitimate"] if v < 0 else COLORS["phishing"] for v in delta.values]
        ax.barh(delta.index, delta.values, color=colors)
        ax.axvline(0, color="gray", linewidth=0.8)
        ax.set_title("Feature Deviation in False Positives (Z-Score vs True Negatives)", fontsize=16, fontweight="bold")
        ax.set_xlabel("Standard Deviations")
        _save(fig, "09_fp_feature_deviation")
        
    # ── 4. CONFIDENCE DISTRIBUTION PLOT ──
    fig, ax = plt.subplots(figsize=(10, 5))
    tp = eval_df[(eval_df["label"] == 1) & (eval_df["pred_label"] == 1)]["confidence"]
    fn = eval_df[(eval_df["label"] == 1) & (eval_df["pred_label"] == 0)]["confidence"]
    
    if len(tp) > 0: sns.kdeplot(tp, ax=ax, label=f"True Positives (n={len(tp)})", color="#1abc9c", fill=True, alpha=0.2)
    if len(fn) > 0: sns.kdeplot(fn, ax=ax, label=f"False Negatives (n={len(fn)})", color="#9b59b6", fill=True, alpha=0.2)
    if len(fp_len) > 0: sns.kdeplot(eval_df[eval_df["is_fp"]]["confidence"], ax=ax, label=f"False Positives (n={len(fp_len)})", color="#e74c3c", fill=True, alpha=0.2)
    if len(tn_len) > 0: sns.kdeplot(eval_df[eval_df["is_tn"]]["confidence"], ax=ax, label=f"True Negatives (n={len(tn_len)})", color="#3498db", fill=True, alpha=0.2)
        
    ax.set_title("Model Confidence Distribution by Outcome", fontsize=16, fontweight="bold")
    ax.set_xlabel("Confidence Score")
    ax.set_xlim(0, 1)
    ax.legend()
    _save(fig, "10_confidence_distribution")
...
def run_eda():
    ...
    logger.info("Step 7/7: Checking for prediction analysis...")
    preds_path = os.path.join(cfg.EVAL_DIR, "val_predictions.csv")
    if os.path.exists(preds_path):
        logger.info(f"Loading predictions from {preds_path}")
        try:
            preds = pd.read_csv(preds_path)
            plot_false_positive_analysis(val_df, preds)
        except Exception as e:
            logger.error(f"Failed to run FP analysis: {e}")
    else:
        logger.info("No val_predictions.csv found, skipping False Positive analysis plots.")
